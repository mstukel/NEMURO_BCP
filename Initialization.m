function [tracer,tracer_init,temperature,salinity,Deep,tmpdeep,Kz_mid,Kz_edge,z,z_edge,z_thick,PAR_surf,wind,iCO2,iThorium,iN15,fastsinkingIndices,slowsinkingIndices,mixedIndices,tot_iter]=Initialization(Cycle,CO2on,Thoriumon,N15on,sim_dur,dt)
...
load('Initial Conditions.mat')
load('Initial Carbon-Nutrients from GLODAP.mat')
...
load('CCE temperatures.mat')
tracer_init=tracer_init(find(tracer_init(:,1)==Cycle(1) & tracer_init(:,2)==Cycle(2)),:);
for i=1:length(tracer_init(:,1))
    ind=find(oxy(:,2)==tracer_init(i,3));
    tracer_init(i,19+3)=oxy(ind,3);
...
z=z(find(z(:,1)==Cycle(1) & z(:,2)==Cycle(2)),:);
z_edge=z_edge(find(z_edge(:,1)==Cycle(1) & z_edge(:,2)==Cycle(2)),:);
z_thick=z_thick(find(z_thick(:,1)==Cycle(1) & z_thick(:,2)==Cycle(2)),:);
